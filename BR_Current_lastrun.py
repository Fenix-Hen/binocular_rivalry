...
expName = 'BR_Current'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/marie/Desktop/Psychopy Test/BR_Project/BR_Current_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Visualization = visual.TextStim(win=win, name='Visualization',
        text=None,
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    Welcome = data.Routine(
        name='Welcome',
        components=[WelcomeText, key_resp],
    )
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('BR_Excel.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 0.75:
            ...
            if CueText.status == STARTED:
                ...
                if tThisFlipGlobal > CueText.tStartRefresh + 0.75-frameTolerance:
        ...
        while continueRoutine and routineTimer.getTime() < 6.0:
            ...
            # if Visualization is starting this frame...
            if Visualization.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Visualization.frameNStart = frameN  # exact frame index
                Visualization.tStart = t  # local t and not account for scr refresh
                Visualization.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Visualization, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Visualization.started')
                # update status
                Visualization.status = STARTED
                Visualization.setAutoDraw(True)
            
            # if Visualization is active this frame...
            ...
            if Visualization.status == STARTED:
                ...
                if tThisFlipGlobal > Visualization.tStartRefresh + 6.0-frameTolerance:
                    ...
                    Visualization.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Visualization.stopped')
                    # update status
                    Visualization.status = FINISHED
                    Visualization.setAutoDraw(False)
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # if BRimages is starting this frame...
            if BRimages.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                BRimages.frameNStart = frameN  # exact frame index
                BRimages.tStart = t  # local t and not account for scr refresh
                BRimages.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(BRimages, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'BRimages.started')
                # update status
                BRimages.status = STARTED
                BRimages.setAutoDraw(True)
            
            # if BRimages is active this frame...
            ...
            if BRimages.status == STARTED:
                ...
                if tThisFlipGlobal > BRimages.tStartRefresh + 1.0-frameTolerance:
                    ...
                    BRimages.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'BRimages.stopped')
                    # update status
